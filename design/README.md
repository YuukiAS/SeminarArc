...
- `Research Reconstruction`（0.2.x 已实现，尚未生成高保真图）
...
- `Reference Candidate Review / Seminar Brief`（0.3.x 已实现，尚未生成高保真图）
...
- `0.2.x` Research Reconstruction 与 `0.3.x` Reference Candidate / Seminar Brief 已按 `UI_SPEC.md` 和 `USER_FLOW.md` 的结构化规格落地；没有高保真 PNG 不阻塞生产实现，但后续 polish 不得偏离 `Academic Archive` 的列表优先、安静、专业方向。
