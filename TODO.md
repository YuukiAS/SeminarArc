...
当前 `0.2.x` 已完成：Room v3 数据基础、本地图像增强、本地 OCR provider/use case、WorkManager-backed processing queue、Reconstruction workspace ViewModel/Compose UI、Windows Emulator connected closeout 均已落地，见 `docs/plans/0.2.x-reconstruction-readiness-plan.md` 和 `prompts/tasks/0.2.x_*_result.md`。

范围：

- 图像旋转、裁边、透视矫正和增强，始终保留原图。
- bundled ML Kit 普通 OCR，覆盖 Latin、Chinese、mixed 和 empty/low-text smoke。
- OCR 文本搜索。
- 照片标签和关键幻灯片筛选。
- ProcessingJob durable queue、重试、取消、幂等和 process-start recovery。
- Room migration，为后续 transcript、OCR 和 references 建立稳定数据层边界。
...
- Notion：Markdown 友好 transcript/summary export foundation 和 block-like local contract 已完成；后续再实现官方 API 页面创建、block 追加和文件上传。
...
- 已完成 `0.5.x_formula_provider_contract`：新增 `FormulaOcrProvider` request/result contract、manual provider、production 默认 unavailable provider 和 JVM contract tests。
...
- 执行 `0.5.x_formula_processing_queue_task`：把 formula region 接入 durable `FORMULA_OCR` processing job、manual provider result 写回和 retry/cancel 状态。
