...
- `0.5.x` Formula / Research Export foundation：已形成 `docs/plans/0.5.x-formula-research-export-plan.md`；Room version `7` 已加入本地公式区域、公式结果、`FormulaDao`、domain model 和 `FORMULA_OCR` job 类型；`FormulaOcrProvider`、manual/unavailable providers、JVM contract tests、Reconstruction workspace 本地公式区域 UI、manual LaTeX durable queue、READY formula results 的 Markdown/Notion-ready export，以及 confirmed references 的 deterministic `references.bib` / `references.ris` ZIP export artifact、单独保存和分享入口已就位。Mathpix live provider、PaddleOCR/pix2tex bundling 和任何 cloud upload 均需后续 credential/backend/license 决策。
