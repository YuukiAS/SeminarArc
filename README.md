...
- `0.5.x` Formula / Research Export foundation：已形成 `docs/plans/0.5.x-formula-research-export-plan.md`；Room version `7` 已加入本地公式区域、公式结果、`FormulaDao`、domain model 和安全占位 `FORMULA_OCR` job 类型；`FormulaOcrProvider`、manual/unavailable providers、JVM contract tests 和 Reconstruction workspace 本地公式区域 UI 已就位。Mathpix live provider、PaddleOCR/pix2tex bundling 和任何 cloud upload 均需后续 credential/backend/license 决策。
