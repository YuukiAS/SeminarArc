...
- 2026-09-08 本 Windows 原生 Codex session 中，sandbox 内直接运行 `D:\Android\Sdk\platform-tools\adb.exe devices -l` 会在到达设备枚举前失败：`Cannot mkdir '\.android': Permission denied`。非 sandbox 且临时设置 `USERPROFILE`、`HOME`、`ANDROID_SDK_HOME` 到 `D:\Code\SeminarArc-emulator\.android-user-home` 后，ADB 可枚举设备，但当前同时看到 protected physical serial `8cc54656` 和 `emulator-5554`，因此 connected Emulator gate 仍不得运行。不要用 `adb kill-server`、transport reset、usbipd 或真机操作绕过。
