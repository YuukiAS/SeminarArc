...
- 2026-09-08 本 Windows 原生 Codex session 中，直接运行 `D:\Android\Sdk\platform-tools\adb.exe devices -l` 会在到达设备枚举前失败：`Cannot mkdir '\.android': Permission denied`。即使临时设置 `USERPROFILE`、`HOME`，或移除 `ANDROID_PREFS_ROOT` 后临时设置 `ANDROID_SDK_HOME=D:\Code\SeminarArc-emulator\.android-user-home`，该 `adb.exe` 仍解析到 `\.android`。在修复该 Windows ADB home 解析问题前，不要声称 connected Emulator gate 已完成；也不要用 `adb kill-server`、transport reset、usbipd 或真机操作绕过。
